...
describe('CohereReranker', () => {
    ...
    beforeEach(() => {
        ...
        mockCohereClient = new CohereClient();
    ...
    it('reranks results using Cohere API', async () => {
        ...
        expect(mockCohereClient.rerank).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('handles Cohere API errors gracefully', async () => {
        ...
        await expect(reranker.rerank('q', [{ id: '1', content: 'c', score: 1, metadata: {} }]))
    ...
    it('uses custom model and maxChunksPerDoc if provided', () => {
        const customReranker = new CohereReranker({ apiKey: 'k', model: 'rerank-multilingual-v3.0', maxChunksPerDoc: 5 });
