...
async function fetchSnapshot(refresh = false): Promise<{
  entries: unknown[];
  summary?: Record<string, number>;
  overview?: Record<string, unknown>;
}> {
...
async function fetchEnv(): Promise<{
  nodeVersion?: string;
  nodeEnv?: string;
...
async function fetchHealth(
  endpoint: string
): Promise<{ status: string; ok: boolean; data?: unknown }> {
  try {
...
function updateStatusBar(
  health: { health?: { ok: boolean }; ready?: { ok: boolean }; startup?: { ok: boolean } },
  stats: { uptimeSeconds?: number }
) {
  ...
  uptime.textContent =
    stats.uptimeSeconds != null ? `Uptime: ${formatUptime(stats.uptimeSeconds)}` : '';
}
...
function updateQuickStats(summary: Record<string, number>) {
  ...
  const parts: string[] = [];
  if ((s.route ?? 0) > 0)
    parts.push(`<span class="quick-stat"><strong>${s.route}</strong> routes</span>`);
  if ((s.module ?? 0) > 0)
    parts.push(`<span class="quick-stat"><strong>${s.module}</strong> modules</span>`);
  if ((s.provider ?? 0) > 0)
    parts.push(`<span class="quick-stat"><strong>${s.provider}</strong> providers</span>`);
  if ((s.cron ?? 0) > 0)
    parts.push(`<span class="quick-stat"><strong>${s.cron}</strong> jobs</span>`);
...
function showOfflineBanner(show: boolean) {
  ...
  if (show && !banner) {
    ...
    banner.innerHTML =
      '<span>App not running. Check that your server is started.</span><button id="retry-btn">Retry</button>';
...
const CARD_ICONS: Record<string, string> = {
  ...
  agent:
    '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="10" rx="2"/><circle cx="12" cy="5" r="2"/><path d="M12 7v4"/></svg>',
  ...
  event:
    '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 2v4"/><path d="M16 2v4"/><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M3 10h18"/><path d="M8 14h.01"/><path d="M12 14h.01"/><path d="M16 14h.01"/></svg>',
  graphql:
    '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v18"/><path d="M5 12h14"/><path d="M3 8l9-5 9 5"/><path d="M3 16l9 5 9-5"/></svg>',
  ...
  kafka:
    '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><path d="M9 9h6"/><path d="M9 13h6"/><path d="M9 17h4"/></svg>',
...
function renderGatewayOverview(data: {
  routes: string[];
  totalRoutes: number;
  metrics?: {
    totalCalls: number;
  ...
  el.innerHTML = `
    ...
    ${
      routes.length
        ? `<div class="overview-gateway-routes"><strong>Routes:</strong> ${routes
            .slice(0, 8)
            .map((r) => `<code>${escapeHtml(r)}</code>`)
            .join(
              ', '
            )}${routes.length > 8 ? ` <span class="muted">+${routes.length - 8} more</span>` : ''}</div>`
        : ''
    }
  `;
...
function renderDiscoveryOverview(data: {
  ...
  const bySvc =
    data.instancesByService && typeof data.instancesByService === 'object'
      ? data.instancesByService
      : {};
  const instancesList = services
    .slice(0, 6)
    .map(
      (s) =>
        `<span class="discovery-service"><code>${escapeHtml(s)}</code>: ${bySvc[s] ?? 0} instance(s)</span>`
    )
    .join(', ');
  el.innerHTML = `
...
function renderResilienceOverview(data: {
  ...
  const statesList = states
    .map(
      (s) =>
        `<span class="cb-state ${(s.state || '').toLowerCase()}"><code>${escapeHtml(s.name)}</code>: ${escapeHtml(s.state)}</span>`
    )
    .join(', ');
  el.innerHTML = `
...
function renderHealth(health: {
  ...
  startup?: { status: string; ok: boolean };
}) {
...
function renderRagExplorer(rag: Record<string, unknown>[]) {
  ...
  container.classList.remove('hidden');
  select.innerHTML = ragList
    .map(
      (r) =>
        `<option value="${escapeHtml(String((r as Record<string, string>).pipelineName ?? ''))}">${escapeHtml(String((r as Record<string, string>).pipelineName ?? ''))}</option>`
    )
    .join('');
  resultsEl.innerHTML = '';
}
...
async function doRagSearch() {
  ...
  try {
    const res = await fetch(
      `${BASE}/rag/${encodeURIComponent(pipeline)}/search?q=${encodeURIComponent(q)}`
    );
...
function renderModuleGraph(modules: Record<string, unknown>[]) {
  ...
  const moduleNames = moduleList
    .map((m) => String((m as Record<string, string>).moduleName ?? ''))
    .filter(Boolean);
  ...
  for (const m of moduleList) {
    ...
    const impList = Array.isArray(imports)
      ? imports
      : typeof imports === 'string'
        ? (imports as string).split(/[,;]/).map((s) => s.trim())
        : [];
    for (const imp of impList) {
      ...
      if (to && (uniqueNames.includes(to) || nodes.has(to))) {
        ...
        if (!nodes.has(to))
          nodes.set(to, {
            x: 80 + (nodes.size % 4) * 120,
            y: 40 + Math.floor(nodes.size / 4) * 60,
          });
      }
    }
  }
...
function renderTable(
  ...
  const ths = columns.map((c) => {
    ...
    const sortCls = options?.sortable
      ? ` sortable ${sortState.col === c ? sortState.dir : ''}`
      : '';
...
async function fetchPromptRender(
  key: string,
  variables: Record<string, unknown>
): Promise<{ rendered: string; tokenEstimate: number }> {
...
async function load(refresh = false) {
  ...
  try {
    ...
    if (overview.gateway)
      safeRender('gatewayOverview', () =>
        renderGatewayOverview(overview.gateway as Parameters<typeof renderGatewayOverview>[0])
      );
    if (overview.discovery)
      safeRender('discoveryOverview', () =>
        renderDiscoveryOverview(overview.discovery as Parameters<typeof renderDiscoveryOverview>[0])
      );
    if (overview.resilience)
      safeRender('resilienceOverview', () =>
        renderResilienceOverview(
          overview.resilience as Parameters<typeof renderResilienceOverview>[0]
        )
      );
    ...
    const [env, health, stats] = await Promise.all([
      fetchEnv(),
      Promise.all([fetchHealth('/health'), fetchHealth('/ready'), fetchHealth('/startup')]).then(
        ([h, r, s]) => ({
          health: h,
          ready: r,
          startup: s,
    ...
    const routes = (snapshot.entries as Record<string, unknown>[]).filter(
      (e) => e.kind === 'route'
    );
    const modules = (snapshot.entries as Record<string, unknown>[]).filter(
      (e) => e.kind === 'module'
    );
    const providers = (snapshot.entries as Record<string, unknown>[]).filter(
      (e) => e.kind === 'provider'
    );
    const jobs = (snapshot.entries as Record<string, unknown>[]).filter((e) => e.kind === 'cron');
    const queues = (snapshot.entries as Record<string, unknown>[]).filter(
      (e) => e.kind === 'queue'
    );
    const ws = (snapshot.entries as Record<string, unknown>[]).filter(
      (e) => e.kind === 'websocket'
    );
    const agents = (snapshot.entries as Record<string, unknown>[]).filter(
      (e) => e.kind === 'agent'
    );
    const rag = (snapshot.entries as Record<string, unknown>[]).filter((e) => e.kind === 'rag');
    const prompts = (snapshot.entries as Record<string, unknown>[]).filter(
      (e) => e.kind === 'prompt'
    );
    const aifunctions = (snapshot.entries as Record<string, unknown>[]).filter(
      (e) => e.kind === 'aifunction'
    );
    const events = (snapshot.entries as Record<string, unknown>[]).filter(
      (e) => e.kind === 'event'
    );
    const graphql = (snapshot.entries as Record<string, unknown>[]).filter(
      (e) => e.kind === 'graphql'
    );
    ...
    const serverless = (snapshot.entries as Record<string, unknown>[]).filter(
      (e) => e.kind === 'serverless'
    );
    ...
    renderTable(
      idToTable['modules'],
      modules,
      ['moduleName', 'dynamicModule', 'imports'],
      'module',
      undefined,
      { sortable: true }
    );
    ...
    renderTable(
      idToTable['rag'],
      rag,
      ['pipelineName', 'vectorStore', 'embeddingProvider', 'chunkingStrategy'],
      'rag',
      undefined,
      { sortable: true }
    );
    renderTable(idToTable['prompts'], prompts, ['promptKey', 'scope'], 'prompt', undefined, {
      sortable: true,
      previewPrompt: true,
    });
    ...
    renderTable(
      idToTable['graphql'],
      graphql,
      ['operationType', 'operationName', 'resolverName', 'className'],
      'graphql'
    );
    ...
    renderTable(
      idToTable['kafka'],
      kafka,
      ['consumerName', 'topic', 'groupId', 'methodName'],
      'kafka'
    );
    ...
    renderTable(
      idToTable['serverless'],
      serverless,
      ['controllerName', 'runtime', 'memory', 'timeout'],
      'serverless'
    );
    ...
    if (container) {
      const msg = isFetchError
        ? 'Is the app running?'
        : 'A rendering error occurred. Check the console for details.';
...
document.getElementById('diff-file')?.addEventListener('change', (e) => {
  ...
  reader.onload = () => {
    (document.getElementById('diff-snapshot-b') as HTMLTextAreaElement).value = String(
      reader.result
    );
  };
...
document.getElementById('diff-compare-btn')?.addEventListener('click', () => {
  ...
  let html = '';
  if (added.length)
    html += `<p class="added">+ ${added.length} added</p><ul>${added.map((e) => `<li>${escapeHtml(String(e.kind))}: ${escapeHtml(String(e.id ?? ''))}</li>`).join('')}</ul>`;
  if (removed.length)
    html += `<p class="removed">− ${removed.length} removed</p><ul>${removed.map((e) => `<li>${escapeHtml(String(e.kind))}: ${escapeHtml(String(e.id ?? ''))}</li>`).join('')}</ul>`;
  if (changed.length)
    html += `<p class="changed">~ ${changed.length} changed</p><ul>${changed.map((e) => `<li>${escapeHtml(String(e.kind))}: ${escapeHtml(String(e.id ?? ''))}</li>`).join('')}</ul>`;
...
document.getElementById('search')!.addEventListener('input', (e) => {
  ...
  renderTable('prompts-table', prompts, ['promptKey', 'scope'], 'prompt', undefined, {
    sortable: true,
    previewPrompt: true,
  });
