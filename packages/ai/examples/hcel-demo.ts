...
async function demonstrateHCEL() {
  ...
  try {
    ...
    const simpleResult = (await ai.hazel.prompt('What is HazelJS?').execute()) as string;
    ...
    try {
      const ragMlResult = (await ai.hazel.rag('docs').ml('sentiment').execute()) as SentimentResult;
    ...
    try {
      const parallelChain = ai.hazel.parallel(
      ...
      console.log(
        `Parallel results: ${Array.isArray(parallelResult) ? parallelResult.length : 1} operations\n`
      );
    } catch (error) {
    ...
    console.log(
      `Chain configured: ${JSON.stringify(configuredChain.getSummary().config, null, 2)}\n`
    );
