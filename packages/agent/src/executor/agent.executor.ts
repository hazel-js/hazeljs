...
export class AgentExecutor {
  ...
  async execute(context: AgentContext, maxSteps: number = 10): Promise<AgentExecutionResult> {
    ...
    try {
      ...
      this.stateManager.updateState(context.executionId, AgentState.THINKING);

      let stepNumber = 0;
      let finalResponse: string | undefined;

      while (this.stateManager.canContinue(context.executionId, maxSteps)) {
        stepNumber++;

        const step = await this.executeStep(context, stepNumber);
        this.stateManager.addStep(context.executionId, step);
      ...
      if (stepNumber >= maxSteps) {
        this.stateManager.updateState(context.executionId, AgentState.FAILED);
        throw new Error(`Maximum steps (${maxSteps}) exceeded`);
      }

      this.stateManager.updateState(context.executionId, AgentState.COMPLETED);
  ...
  async resume(executionId: string, input?: string): Promise<AgentExecutionResult> {
    const context = this.stateManager.getContext(executionId);
    if (!context) {
      throw new Error(`Execution context ${executionId} not found`);
    }

    if (input) {
      this.stateManager.addMessage(executionId, 'user', input);
      this.emitEvent(AgentEventType.USER_INPUT_RECEIVED, executionId, {
        response: input,
      });
    }

    this.stateManager.updateState(executionId, AgentState.THINKING);
