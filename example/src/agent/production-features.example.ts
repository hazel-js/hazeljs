...
async function demonstrateEventMonitoring(runtime: AgentRuntime) {
  ...
  runtime.on(AgentEventType.EXECUTION_STARTED, (event: any) => {
    events.push('EXECUTION_STARTED');
    console.log('📢 Event: Execution started');
  });

  runtime.on(AgentEventType.EXECUTION_COMPLETED, (event: any) => {
  ...
  runtime.on(AgentEventType.TOOL_EXECUTION_COMPLETED, (event: any) => {
